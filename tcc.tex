\input{fixos/pacotes}
\input{fixos/comandos}
\input{fixos/novosComandos}

\input{editaveis/informacoes}
\input{fixos/informacoes}
\input{fixos/setup}

\begin{document}

\frenchspacing
\imprimircapa
\imprimirfolhaderosto*

% \input{fixos/fichaCatalografica}
%\input{editaveis/errata}
% \input{fixos/folhaDeAprovacao}
%\input{editaveis/dedicatoria}
% \input{editaveis/agradecimentos}
% \input{editaveis/epigrafe}
% \input{editaveis/resumo}
% \input{editaveis/abstract}
\input{fixos/listasAutomaticas}
% \input{editaveis/abreviaturas}
% \input{editaveis/simbolos}
\input{fixos/indiceAutomatico}

\textual

\input{editaveis/introducao}
\input{editaveis/solucao_requisitos}
\input{editaveis/solucao_alimentacao}
\input{editaveis/solucao_estrutura}
\input{editaveis/solucao_eletronico}
\input{editaveis/solucao_comunicacao}
\input{editaveis/solucao_controle}
% \input{editaveis/objetivos}
% \input{editaveis/fundamentacaoteorica}
% \input{editaveis/delimitacaodoassunto}
% \input{editaveis/metodologia}
% \input{editaveis/resultadosalcancados}
% \input{editaveis/cronogramadedesenvolvimento}

\bookmarksetup{startatroot}

\postextual

\bibliography{bibliografia}
%\input{editaveis/apendices}
% \input{editaveis/anexos}
\printindex

\end{document}
